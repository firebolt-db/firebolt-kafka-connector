...
public class ParquetAvroSchemaProvider {
    ...
    private Schema mapSqlTypeToAvro(TableSchema.Column tableColumn) {
        ...
        switch (tableColumn.getSqlType()) {
            ...
            case Types.ARRAY:
                ...
                break;
            case Types.NUMERIC:
                base = LogicalTypes.decimal(tableColumn.getPrecision(), tableColumn.getScale())
    ...
    private Schema createArraySchema(TableSchema.Column tableColumn) {
        ...
        boolean isTextArray = dataType.startsWith("array(text");
        boolean isBooleanArray = dataType.startsWith("array(boolean");
        boolean isByteaArray = dataType.startsWith("array(bytea");
        ...
        if (isIntArray) {
            ...
            Schema decimal = LogicalTypes.decimal(tableColumn.getPrecision(), tableColumn.getScale())
                    .addToSchema(Schema.create(Schema.Type.BYTES));
            ...
            itemSchema = SchemaBuilder.unionOf().nullType().and().booleanType().endUnion();
        } else if (isByteaArray) {
            itemSchema = SchemaBuilder.unionOf().nullType().and().bytesType().endUnion();
        } else {
            itemSchema = SchemaBuilder.unionOf().nullType().and().stringType().endUnion();
        }
